...
ALightDetector::ALightDetector()
{
    ...
    MainCapsule = CreateDefaultSubobject<UCapsuleComponent>(TEXT("Root"));
    RootComponent = MainCapsule;

    TopCapture = CreateDefaultSubobject<USceneCaptureComponent2D>(TEXT("TopCature"));
    TopCapture->SetupAttachment(RootComponent);

    BottomCapture = CreateDefaultSubobject<USceneCaptureComponent2D>(TEXT("BottomCature"));
    BottomCapture->SetupAttachment(RootComponent);

    TopCapture->TextureTarget = detectorTextureTop;
    BottomCapture->TextureTarget = detectorTextureBottom;
